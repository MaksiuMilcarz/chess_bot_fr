...
def simulate_game(model_1, model_2, move_to_int, initial_board=None, sleep_time=0.5, board_size=500):
    """
    Simulates a game between two models and displays the board after each move.
    
    Parameters:
    - model_white: The model controlling the white pieces.
    - model_black: The model controlling the black pieces.
    - move_to_int: Dictionary mapping moves in UCI format to integer indices.
    - initial_board: Optional starting board position (chess.Board object). If None, starts from the default position.
    - sleep_time: Time in seconds to wait between moves for display purposes.
    - board_size: Size of the displayed board in pixels.
    """
    ...
    while True:
        ...
        if board.turn == chess.WHITE:
            best_move = predict_best_move(model_1, move_to_int, board)
        else:
            best_move = predict_best_move(model_2, move_to_int, board)
